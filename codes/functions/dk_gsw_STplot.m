%% Recreated/Edited by Dolly More
...
if ~isempty(isopycs)
    [c1,h] = contour(SA_gridded,CT_gridded,isopycs_gridded,round(isopycs,2),':','Color',[.5 .5 .5]);
end
hold on;
...
if exist('c1','var')
    clabel(c1,h,'labelspacing',360,'fontsize',10*sz,'color','black');
end
...
if exist('title_string','var')
    ...
    title('TS diagram','fontsize',13*sz); %\it{S}\rm_A - {\Theta}
end
set(gca,'tickdir','out');
